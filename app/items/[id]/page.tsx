        ...
        <div className="max-w-xl mx-auto px-6 py-8 flex flex-col gap-6">
          ...
          <div className="flex items-center justify-between bg-white/60 backdrop-blur-sm rounded-2xl px-5 py-4 border border-[#D9CFC4]">
            <div className="flex items-center gap-3">
              ...
              <div>
                ...
                <p className="text-xs text-[#A09080]">{t("item.viewListings")}</p>
              </div>
            </div>
            <Link
          ...
          {userId && item.owner_id !== userId && (
            <div className="flex flex-col gap-3">
              ...
              <Link
                href={`/messages/${item.owner_id}`}
                className="w-full rounded-full border-2 border-[#4A3728] text-[#4A3728] py-3.5 font-semibold hover:bg-[#4A3728] hover:text-[#F5F0E8] transition-colors flex items-center justify-center gap-2"
              >
                <svg viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round" className="w-4 h-4">
                  <path d="M21 15a2 2 0 0 1-2 2H7l-4 4V5a2 2 0 0 1 2-2h14a2 2 0 0 1 2 2z" />
                </svg>
                {t("item.messageOwner", { name: item.ownerName.split(" ")[0] })}
              </Link>
            </div>
          )}
          ...
          {/* Description */}
          <div>
            <h3 className="text-sm font-medium text-[#4A3728] mb-2">{t("item.description")}</h3>
            ...
            <div>
              <div className="flex items-center justify-between mb-3">
                <h3 className="text-sm font-medium text-[#4A3728]">
                  {t("item.moreFrom", { name: "" })}<Link href={`/members/${item.owner_id}`} className="hover:underline">{item.ownerName}</Link>
                </h3>
                <Link href={`/members/${item.owner_id}`} className="text-xs text-[#8B7355] hover:underline">{t("item.seeAll")}</Link>
              </div>
