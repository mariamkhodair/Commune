...
export default function ProposeSwapModal({
  ...
  useEffect(() => {
    if (!visible || !proposerId || !receiverId) return;
    setSubmitted(false);
    setLoading(true);
    ...
    async function fetchData() {
      const [{ data: itemsData }, { data: wantedData }] = await Promise.all([
        supabase.from("items").select("id, name, category, points")
          .eq("owner_id", proposerId).eq("status", "Available"),
    ...
    fetchData();
  }, [visible, proposerId, receiverId, theirTotal]);
  ...
  return (
    <Modal visible={visible} animationType="slide" transparent onRequestClose={onClose}>
      <View style={{ flex: 1, backgroundColor: "rgba(74,55,40,0.35)", justifyContent: "flex-end" }}>
        <View style={{ backgroundColor: "#FAF7F2", borderTopLeftRadius: 28, borderTopRightRadius: 28, maxHeight: "90%", paddingBottom: 34 }}>
                  ...
                  <>
                    ...
                    <View style={{ backgroundColor: "#D8E4D0", borderRadius: 12, padding: 12, flexDirection: "row", gap: 10, alignItems: "flex-start" }}>
                      ...
                      <View style={{ flex: 1 }}>
                        ...
                        <Text style={{ fontSize: 11, color: "#4A6640", lineHeight: 16 }}>We've pre-selected the combination of your items closest in value.</Text>
                      </View>
                    ...
                    {theirWanted.length > 0 && (
                      <View style={{ backgroundColor: "#F5F0E8", borderRadius: 12, padding: 12 }}>
                        <Text style={{ fontSize: 12, color: "#6B5040", lineHeight: 18 }}>
                          ...
                          {". Items marked 🤝🏽 match what they want."}
                        </Text>
                      </View>
                    )}
