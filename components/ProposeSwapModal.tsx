...
function suggestBundle(target: number, items: MyItem[]): Set<string> {
  const capped = items.slice(0, 18); // cap for bitmask perf
  ...
  for (let mask = 1; mask < (1 << n); mask++) {
    ...
    for (let i = 0; i < n; i++) {
      if (mask & (1 << i)) {
        subset.push(capped[i].id);
        total += capped[i].points;
      }
    }
...
export default function ProposeSwapModal({
  ...
  const receiverId = items[0]?.ownerId;
  ...
  async function handleSubmit() {
    ...
    const swapItems = [
      ...
      ...items.map((i) => ({ swap_id: swapData.id, item_id: i.id })),
    ];
  ...
  const satisfiesTheirWant =
    ...
    selectedItems.some((i) =>
      theirWanted.some(
        (w) => i.name.toLowerCase().includes(w) || w.includes(i.name.toLowerCase())
      )
    );
    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center px-4">
      ...
      <div className="relative w-full max-w-md bg-[#FAF7F2] rounded-3xl px-7 py-8 shadow-lg max-h-[90vh] overflow-y-auto">
        ...
        <div className="bg-[#EDE8DF] rounded-2xl px-4 py-4 mb-5">
          ...
          <div className="flex items-center justify-between mt-2 pt-2 border-t border-[#C4B9AA]">
            <p className="text-xs text-[#8B7355]">
              Listed by{" "}
              {receiverId
                ? <Link href={`/members/${receiverId}`} className="hover:underline font-medium text-[#4A3728]">{ownerName}</Link>
                : ownerName}
            </p>
        ...
        {theirWanted.length > 0 && (
          <div className="flex items-start gap-2 bg-[#F5F0E8] rounded-xl px-4 py-3 mb-5">
            ...
            <p className="text-xs text-[#6B5040] leading-relaxed">
              ...
              <span className="font-medium text-[#4A3728]">{theirWanted.join(", ")}</span>. Items marked 🤝🏽 match what they want.
            </p>
          </div>
        )}
        ...
        {loadingItems ? (
          ...
          <div className="bg-[#ECD8D4] rounded-xl px-4 py-4 text-center mb-5">
            <p className="text-sm font-medium text-[#8B3A2A] mb-1">You have no listed items</p>
            <p className="text-xs text-[#8B3A2A]">List some items first before proposing a swap.</p>
          </div>
        ) : (
          <>
            ...
            <div className="flex items-start gap-2 bg-[#D8E4D0] rounded-xl px-4 py-3 mb-4">
              ...
              <div className="flex-1">
                ...
                <p className="text-xs text-[#4A6640] leading-relaxed">
                  We've pre-selected the combination of your items closest in value to {ownerName}'s listing.
                </p>
              </div>
              <button
            ...
            <div className="flex flex-col gap-2 mb-5">
              {myItems.map((myItem) => {
                ...
                const wantedByThem = theirWanted.some(
                  (w) => myItem.name.toLowerCase().includes(w) || w.includes(myItem.name.toLowerCase())
                );
                return (
                  <button
                    key={myItem.id}
        ...
        {error && <p className="text-xs text-[#8B3A2A] mb-3 text-center">{error}</p>}

        <button
          onClick={handleSubmit}
          disabled={selected.size === 0 || submitting || loadingItems || myItems.length === 0}
          className="w-full rounded-full bg-[#4A3728] text-[#F5F0E8] py-3 font-semibold hover:bg-[#6B5040] transition-colors disabled:opacity-40 disabled:cursor-not-allowed flex items-center justify-center gap-2"
        >
          {submitting ? (
            <><div className="w-4 h-4 border-2 border-[#F5F0E8] border-t-transparent rounded-full animate-spin" /> Sending…</>
          ) : "Send Proposal"}
        </button>
